...
public interface OrderContextDataMapper {
	...
	@Select("select partition_id as partitionId, order_id as orderId, data_key as dataKey, "
			...
			+ " from order_context__data where order_id=#{orderId} and data_key=#{dataKey} and category=#{category}")
	OrderContextData selectByOrderIdAndDataKey(@Param("category") String category,@Param("orderId") String orderId, @Param("dataKey") String dataKey);

	
	
	@Select("select count(*) from order_context__data where order_id=#{orderId} and data_key=#{dataKey} and category=#{category}")
	int selectCount(@Param("category") String category,@Param("orderId") String orderId, @Param("dataKey") String dataKey);
	...
	// //根据key更新上下文数据
	@Update("update order_context__data set step_id=#{stepId}, flow_id=#{flowId}, context_data=#{contextDataBlob} where order_id=#{orderId} and data_key=#{dataKey} and category=#{category}")
